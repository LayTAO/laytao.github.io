---
permalink: /
title: ""
excerpt: "Tao Lei — incoming M.Phil. student at HKUST (Guangzhou), researching LLMs for software engineering."
author_profile: true
redirect_from:
  - /about/
  - /about.html
---

<span class="anchor" id="about-me"></span>

<div class="intro-card" markdown="1">

<div class="eyebrow">LLM4SE · MUTATION TESTING · TEST GENERATION</div>

## Hi, I'm Tao Lei.

I am an incoming M.Phil. student at **The Hong Kong University of Science and Technology (Guangzhou)**. My research sits at the intersection of **large language models** and **software engineering**, with a particular interest in building reliable and efficient techniques for software testing.

I received my B.E. in **Software Engineering** from **Xuzhou University of Technology**. My undergraduate research focused on high-precision test-data generation for mutation testing. From December 2025 to March 2026, I worked as a Research Assistant at the **Systems Hub, HKUST (Guangzhou)**.

I am always happy to discuss research ideas and potential collaborations. Feel free to reach me by [email](mailto:weaslypaper@163.com).

</div>

### Research Interests

<div class="research-tags">
  <span>Large Language Models</span>
  <span>LLMs for Software Engineering</span>
  <span>Mutation Testing</span>
  <span>Test-Data Generation</span>
  <span>AI-driven Defect Prediction</span>
</div>

<span class="anchor" id="news"></span>

# 🔥 News

- **2026.09** — I will begin my M.Phil. study at HKUST (Guangzhou), focusing on LLMs for software engineering.
- **2026.03** — I completed my research assistantship at the Systems Hub, HKUST (Guangzhou).
- **2025.12** — Our paper on efficient multi-path coverage in mutation testing was published in *Mathematics*.
- **2025.08** — Our invention patent on CNN- and genetic-algorithm-based software testing was granted.
- **2025.06** — Our paper on SI_CNNpro-enhanced test-data generation was published in *Journal of Systems and Software*.

<span class="anchor" id="publications"></span>

# 📝 Selected Publications

<div class="paper-box">
  <div class="paper-box-image">
    <div class="paper-visual paper-visual--blue">
      <span>JSS 2025</span>
      <strong>Learning-guided Test Generation</strong>
    </div>
  </div>
  <div class="paper-box-text" markdown="1">

### [Optimizing Test Data Generation Using SI_CNNpro-Enhanced MGA for Mutation Testing](https://doi.org/10.1016/j.jss.2025.112517)

**Lei Tao**, Xiangying Dang, Juxin Hu, Dunwei Gong, Guosheng Hao, Xiangjuan Yao, Bingsen Huang

*Journal of Systems and Software*, 2025.  
[[Paper]](https://doi.org/10.1016/j.jss.2025.112517)

<p class="paper-note">A learning-guided evolutionary framework for generating high-quality mutation-testing data while reducing redundant search.</p>

  </div>
</div>

<div class="paper-box">
  <div class="paper-box-image">
    <div class="paper-visual paper-visual--teal">
      <span>MATHEMATICS 2026</span>
      <strong>Multi-path Mutation Coverage</strong>
    </div>
  </div>
  <div class="paper-box-text" markdown="1">

### [Efficient Multiple Path Coverage in Mutation Testing with Fuzzy Clustering-Integrated MF_CNNpro_PSO](https://www.mdpi.com/2227-7390/14/1/47)

Qian Qu, Xiangying Dang, Heng Xia, **Lei Tao**

*Mathematics*, 2026, 14(1), 47.  
[[Paper]](https://www.mdpi.com/2227-7390/14/1/47)

<p class="paper-note">Combines fuzzy clustering, a multi-feature CNN surrogate, and particle swarm optimization for efficient multi-path test generation.</p>

  </div>
</div>

<span class="anchor" id="patents"></span>

# 📜 Patents

- **Software Testing Method for Intelligent Equipment Combining CNN and Genetic Algorithm.** **Lei Tao**, Xiangying Dang, Bingsen Huang, Tianhao Chen. Authorized Chinese invention patent, **CN 119988236 B**, 2025. [[Patent PDF]](https://weaslypaper-coder.github.io/LT/files/%E5%8F%91%E6%98%8E%E4%B8%93%E5%88%A92.pdf)
- **Scheduling Method of Blocking Water Turbines Based on Improved Genetic Algorithm.** **Lei Tao**, Xiangying Dang. Authorized Chinese invention patent, **CN 118014316 B**, 2024. [[Patent PDF]](https://weaslypaper-coder.github.io/LT/files/%E5%8F%91%E6%98%8E%E4%B8%93%E5%88%A91.pdf)

<span class="anchor" id="education"></span>

# 🎓 Education

- **M.Phil. student**, The Hong Kong University of Science and Technology (Guangzhou), incoming September 2026.
- **B.E. in Software Engineering**, Xuzhou University of Technology.

<span class="anchor" id="talks"></span>

# 🎙 Selected Talks

- **2025.01** — *When UAV Meets LLMs: Intelligent Airborne Computing*, HKUST (Guangzhou), Guangzhou, China.
- **2024.07** — *Research on Solution Algorithms for Mutation Testing Data Generation*, online academic sharing. [[Video]](https://b23.tv/lDVFSfv)
- **2024.07** — *Introduction to Mutation Testing and Research Directions*, online academic sharing. [[Video]](https://b23.tv/YMqnkZq)

<span class="anchor" id="experience"></span>

# 💻 Research Experience

- **Research Assistant**, Systems Hub, The Hong Kong University of Science and Technology (Guangzhou), December 2025 – March 2026.

<div class="contact-card" markdown="1">

### Let's connect

I welcome conversations about LLM4SE, mutation testing, and search-based software engineering.  
[Email](mailto:weaslypaper@163.com) · [Google Scholar](https://scholar.google.com/citations?hl=en&user=J4SIspEAAAAJ) · [GitHub](https://github.com/weaslypaper-coder) · [ORCID](https://orcid.org/0009-0008-4014-4013)

</div>
